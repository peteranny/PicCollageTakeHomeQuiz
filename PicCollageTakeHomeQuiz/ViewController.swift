...
class ViewController: UIViewController {
    ...
    private func installBindings() {
        ...
        let bindKeyboardDismissal = fontButton.tapPublisher
            .sink { [base = textView] _ in
                base.resignFirstResponder()
            }

        // Toggle the font selector on tapping the font button
        let bindFontSelectorInstallation = fontButton.tapPublisher
    ...
    private lazy var fontSelectorViewModel: FontSelectorViewModel = {
        ...
        return FontSelectorViewModel(manager: manager, fontObserver: fontBinder)
    }()
